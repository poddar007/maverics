...
func (a *Algorithm) Sign(unsignedToken string) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, errors.New("Unable to write to HMAC-SHA256"))
	}
...
func (a *Algorithm) Encode(payload *Claims) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, errors.New("unable to marshal header"))
	}
	...
	if err != nil {
		return "", errors.Wrap(err, errors.New("Failed to create JSON payload"))
	}
	...
	if err != nil {
		return "", errors.Wrap(err, errors.New("Failed to sign payload"))
	}
...
func (a *Algorithm) DecodeAndValidate(encoded string) (claims *Claims, err error) {
	...
	if err = a.validateSignature(encoded); err != nil {
		err = errors.Wrap(err, errors.New("failed to validate signature"))
		return
	}

	if err = a.validateExp(claims); err != nil {
		err = errors.Wrap(err, errors.New("failed to validate exp"))
		return
	}

	if err = a.validateNbf(claims); err != nil {
		err = errors.Wrap(err, errors.New("failed to validate nbf"))
	}
...
func (a *Algorithm) validateSignature(encoded string) error {
	...
	if err != nil {
		return errors.Wrap(err, errors.New("unable to sign token for validation"))
	}
